...
**https://selfspeed.app is live on Cloudflare Pages.** All 15 content pages plus
the app, sitemap, robots, icons and service worker verified serving 200. The app
boots clean in a headless browser with zero console errors.

- [x] **Cloudflare Pages** — free, custom domain, TLS issued in minutes
- [x] DNS on Cloudflare, HTTPS working
- [x] `sitemap.xml` + `robots.txt` — generated by the build
- [ ] **Turn on "Always Use HTTPS"** in Cloudflare → SSL/TLS → Edge Certificates.
      `http://selfspeed.app/` currently returns 200 instead of redirecting.
      `.app` is HSTS-preloaded so browsers force HTTPS anyway, but non-browser
      clients and crawlers should get a 301.
- [ ] **Google Search Console** — add property, submit the sitemap. Indexing
      takes weeks, so do it before anything else.
- [ ] Cloudflare Web Analytics — free, one toggle, no cookie banner needed
      (unlike GA4, which would need consent)

### Why not GitHub Pages

GitHub failed to provision a TLS certificate across three attempts over two
days, never advancing past certificate state `new`. Certificate Transparency
logs confirmed Let's Encrypt was never even asked — the failure was entirely
upstream of domain validation, on GitHub's side. DNS, CAA, the ACME challenge
path over both IPv4 and IPv6, and the site itself all checked out.

One genuine misconfiguration was found and fixed along the way: the `www` CNAME
pointed at the apex rather than at the Pages host, so GitHub answered the ACME
challenge with a redirect that discarded the challenge path. Worth knowing if
GitHub Pages is ever used again; it was not the root cause here.
